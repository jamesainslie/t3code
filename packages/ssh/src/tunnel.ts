...
T3_RUNTIME_DIR="$HOME/${FORK_IDENTITY.baseDirName}/runtime/versions/$T3_ARCHIVE_VERSION"
t3_runtime_ready() {
  [ -x "$T3_RUNTIME_DIR/t3" ] && [ "$(cat "$T3_RUNTIME_DIR/.install-complete" 2>/dev/null)" = "$T3_ARCHIVE_VERSION" ]
}
if ! t3_runtime_ready; then
  mkdir -p "$HOME/${FORK_IDENTITY.baseDirName}/runtime/versions"
  ...
  # under it.
  T3_LOCK="$HOME/${FORK_IDENTITY.baseDirName}/runtime/versions/.$T3_ARCHIVE_VERSION.install.lock"
...
if ! t3_runtime_ready; then
  ...
  T3_STAGING="$(mktemp -d "$HOME/${FORK_IDENTITY.baseDirName}/runtime/versions/.staging-XXXXXX")"

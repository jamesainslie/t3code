...
export function parseProbeOutput(output: string): ProbeResult | null {
  // VERSION field may contain spaces (e.g., "t3 v0.0.15") so match rest of line
  const match = output.trim().match(/^OS:(\S+)\s+ARCH:(\S+)\s+VERSION:(.*)$/);
...
export function buildTmuxStartCommand(projectId: string, workspaceRoot: string): string {
  ...
  //   t3 serve --host 127.0.0.1 --auth-token-file <file> --state-file <file>
  //            --log-dir <dir> --headless <cwd> > <log> 2>&1
  // Notes:
  //   - No --port flag: let server pick default (port 0 is rejected by schema)

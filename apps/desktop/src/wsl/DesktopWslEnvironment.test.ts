...
describe("WSL runtime cache", () => {
  ...
  it("parses only absolute Linux runtime paths", () => {
    expect(
      parseWslRuntimeRoot(
        `runtimeRoot:/home/josh/${FORK_IDENTITY.baseDirName}/wsl-runtime/1.2.3-x64\n`,
      ),
    ).toBe(`/home/josh/${FORK_IDENTITY.baseDirName}/wsl-runtime/1.2.3-x64`);
...
describe.skipIf(posixShellRunner === null)("WSL runtime install script (executed)", () => {
  ...
  const createFixture = () => {
    ...
    return {
      ...
      serverEntry: `${work}/home/${FORK_IDENTITY.baseDirName}/wsl-runtime/${runtimeId}/apps/server/dist/bin.mjs`,
      installScript,

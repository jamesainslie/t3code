...
const UNIT_FILE = `${FORK_IDENTITY.bootService.systemdName}.service`;
const LAUNCHD_LABEL = FORK_IDENTITY.bootService.launchdLabel;
...
it("keeps systemd pinned to the stable launcher rather than a versioned server", () => {
  const unit = BootService.renderBootServiceUnit({
    ...
    unitPath: `/home/theo/.config/systemd/user/${UNIT_FILE}`,
  });
...
const macPlan = {
  ...
  unitPath: `/Users/theo/Library/LaunchAgents/${LAUNCHD_PLIST_FILE}`,
};
  ...
  const runner = ProcessRunner.ProcessRunner.of({
    run: Effect.fn("test.run_boot_service_command")(function* (
      ...
      if (!failed && command === "loginctl enable-linger --no-ask-password 501")
        control.linger = "yes";
      if (!failed && command === `systemctl --user enable ${UNIT_FILE}`) control.enabled = true;
      if (!failed && command === `systemctl --user restart ${UNIT_FILE}`) control.active = true;
      if (
        control.stateAfterStop !== undefined &&
        (command === `systemctl --user stop ${UNIT_FILE}` ||
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(commands).not.toContain(`systemctl --user stop ${UNIT_FILE}`);
      }),
  );
  ...
  it.effect("installs, reports current state, and uninstalls", () =>
    Effect.gen(function* () {
      ...
      expect(timeouts.get(`systemctl --user disable --now ${UNIT_FILE}`)).toEqual(
    ...
    (platform) =>
      Effect.gen(function* () {
        ...
        expect(
          ...
          platform === "linux"
            ? [`systemctl --user stop ${UNIT_FILE}`, `systemctl --user restart ${UNIT_FILE}`]
            : [
                `launchctl bootout --wait gui/501/${LAUNCHD_LABEL}`,
  ...
  it.effect("restarts without overwriting a pending remote update", () =>
    Effect.gen(function* () {
      ...
      for (const allowDowngrade of [false, true]) {
        ...
        ).toEqual([`systemctl --user stop ${UNIT_FILE}`, `systemctl --user restart ${UNIT_FILE}`]);
      }
    }),
  );
  ...
  it.effect("installs, reports current state, and uninstalls on macOS", () =>
    Effect.gen(function* () {
      ...
      expect(plan.unitPath.endsWith(path.join("Library", "LaunchAgents", LAUNCHD_PLIST_FILE))).toBe(
        true,
      );
      ...
      expect(timeouts.get(`launchctl bootout --wait gui/501/${LAUNCHD_LABEL}`)).toEqual(
        Duration.seconds(120),
      );
    }),
  );
  ...
  it.effect("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
      ...
      yield* service.install();
      control.failCommand = `launchctl bootout --wait gui/501/${LAUNCHD_LABEL}`;

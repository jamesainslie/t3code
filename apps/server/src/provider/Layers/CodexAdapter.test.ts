...
validationLayer("CodexAdapterLive validation", (it) => {
  ...
  it.effect("maps codex model options before starting a session", () =>
    Effect.gen(function* () {
      ...
      const actual = validationRuntimeFactory.factory.mock.calls[0]?.[0];
      const { extraEnv: _extraEnv, ...deterministicActual } = actual ?? {};
      assert.deepStrictEqual(deterministicActual, {
        binaryPath: "codex",
        cwd: process.cwd(),
        model: "gpt-5.3-codex",
        serviceTier: "fast",

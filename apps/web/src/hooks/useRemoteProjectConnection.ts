 ...
 * - Subscribes to SSH phase updates from the desktop bridge
...
export function useRemoteProjectConnection(project: OrchestrationProject | null | undefined): void {
  ...
  // Subscribe to granular SSH provisioning phase updates from desktop bridge.
  useEffect(() => {
    if (!project?.remoteHost) return;
    if (typeof window === "undefined" || !window.desktopBridge) return;

    const projectId = project.id;
    window.desktopBridge.onSshStatusUpdate((update) => {
      if (update.projectId === projectId) {
        useRemoteConnectionStore.getState().setPhase(projectId, update.phase);
      }
    });
  }, [project?.id, project?.remoteHost]);

...
export type DocsPanelMode = "inline" | "sheet" | "sidebar";

function getDocsPanelHeaderRowClassName(mode: DocsPanelMode) {
...
export function DocsPanelShell(props: {
  mode: DocsPanelMode;
  header: ReactNode;
  children: ReactNode;
}) {
  ...
  return (
    ...
    >
      {shouldUseDragRegion ? (
        <div className={getDocsPanelHeaderRowClassName(props.mode)}>{props.header}</div>
      ) : (
...
export function DocsPanelHeaderSkeleton() {
  return (
    <>
...
export function DocsPanelLoadingState(props: { label: string }) {
  return (
